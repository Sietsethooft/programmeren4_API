...
const participantController = {
    ...
    getParticipantById: (req, res, next) => { // UC-404
        const mealId = parseInt(req.params.mealId, 10);
        ...
        mealService.getMealById(mealId, (error, meals) => {
            ...
            participantService.getParticipantById(mealId, participantId, (error, participant) => {
                if (error) return next(error); // This sends the error to the error handler in util.

                if (!participant) { // Check if the participant exists
                    logger.warn(`Participant with ID #${participantId} not found for meal with ID #${mealId}`);
                    return res.status(404).json({
                        status: 404,
                ...
                logger.info(`Fetched participant with ID #${participantId} for meal with ID #${mealId}`);
                res.status(200).json({
                    status: 200,
